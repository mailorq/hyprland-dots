...
| btop | доступная команда `btop` | интерактивный монитор процессов и ресурсов по `Super + Escape` |
...
Все разворачиваемые текстовые файлы, включая `assets/deployment.sha256`, имеют LF-окончания строк и закреплены в `.gitattributes`. Это сохраняет byte-oriented проверку `sha256sum -c` независимой от платформы, с которой был подготовлен checkout.
