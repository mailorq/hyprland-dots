...
|---|---:|---:|---|
| <code>fm-016</code> | 2048 x 1152 | 20.9% | сгенерированный; стандартный фон |
...
| <code>fm-040</code> | 2560 x 1440 | 21.9% | вручную подготовленный экспорт |

<code>tools/build_wallpapers.py</code> создаёт только <code>fm-016</code>:
используется центральное кадрирование из утверждённого мастера, без
масштабирования. Четыре QHD-файла являются курируемыми экспортами. Сборщик
требует их наличия, проверяет RGB и разрешение 2560 x 1440, но не открывает
их для записи. Для всех файлов манифест фиксирует исходный мастер, контрольные
суммы, координаты утверждённого кадра, размер и режим экспорта.
